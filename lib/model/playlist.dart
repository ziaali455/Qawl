...
class QawlPlaylist {
  ...
  static Future<QawlPlaylist> getStylesPlaylist(String style) async {
    try {
      // Step 1: Get the current user's gender and ID
      QawlUser? currentUser = await QawlUser.getCurrentQawlUser();
      String? currentUserGender = currentUser?.gender;
      String? currentUserId = currentUser?.id;

      if (currentUserGender == null || currentUserId == null) {
        print("Current user gender or ID is null.");
        return QawlPlaylist(
          author: "System",
          name: "Error Playlist",
          list: [],
      ...
      // Step 4: Filter tracks manually based on user ID (only for male users), and exclude current user's uploads
      List<Track> tracks = trackQuerySnapshot.docs
          .where((doc) =>
              (currentUserGender == 'f' || userIds.contains(doc['userId'])) &&
